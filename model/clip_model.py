...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.proj is not None:
            x = x @ self.proj#8 193 768 x 768 512
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x = x @ self.text_projection#8 77 512 x 512 512

        return x

    def forward(self, image, text):
        image_features = self.encode_image(image)#64 3 384 128,  64 193 512
        text_features = self.encode_text(text)#64 77,   64  77  512
